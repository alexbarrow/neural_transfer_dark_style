...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    mod = nn.Sequential(normalization)

    i = 0  # counter for conv layer
    ...
    for layer in cnn.children():
        ...
        mod.add_module(name, layer)

        if name in content_layers:
            target = mod(content_img).detach()
            content_loss = ContentLoss(target)
            mod.add_module("content_loss_{}".format(i), content_loss)
            content_losses.append(content_loss)

        if name in style_layers:
            target_feature = mod(style_img).detach()
            style_loss = StyleLoss(target_feature)
            mod.add_module("style_loss_{}".format(i), style_loss)
            style_losses.append(style_loss)

        # now we trim off the layers after the last content and style losses
    for i in range(len(mod) - 1, -1, -1):
        if isinstance(mod[i], ContentLoss) or isinstance(mod[i], StyleLoss):
            break
